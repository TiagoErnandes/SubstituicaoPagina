...
//Trabalho 2
var novoValorPiline = [];
var estudoValorPiline = [];
var bolhaAdiatamento = [];
var reodernamento = [];
let cont = 0;

const apenasBolha = function () {
    let verificar = valorTratado();
    console.log(verificar)
    while (cont < verificar.length) {
        novoValorPiline.push(verificar[cont]);

        if (cont < verificar.length - 1) {
            if (verificar[cont][0] == 'bne' || verificar[cont][0] == 'beq') {
                novoValorPiline.push("NOP");
                novoValorPiline.push("NOP");
                novoValorPiline.push("NOP");

            } else if ((verificar[cont][1] == verificar[cont + 1][2]) || (verificar[cont][1] == verificar[cont + 1][3].replace(/(\r\n|\n|\r)/gm, ""))) {
                novoValorPiline.push("NOP");
                novoValorPiline.push("NOP");
            }
        } else
            if (cont < (verificar.length - 2)) {
                if (verificar[cont][0] == 'bne' || verificar[cont][0] == 'beq') {
                    novoValorPiline.push("NOP");
                    novoValorPiline.push("NOP");
                    novoValorPiline.push("NOP");

                } else if (verificar[cont][1] == verificar[cont + 2][2] || verificar[cont][1] == verificar[cont + 2][3].replace(/(\r\n|\n|\r)/gm, "")) {
                    novoValorPiline.push("NOP");
                }
            }
...
function bolhaAdiatamentoR() {
    novoValorPiline.forEach(bolha => {

        if (bolha == 'NOP') {

        } else {
            bolhaAdiatamento.push(bolha)
        }
        if (bolha[0] == 'lw') {
            bolhaAdiatamento.push('NOP')
        }
...
function reodernamentoR() {
    bolhaAdiatamento.forEach(reor => {
        ...
        if (reor[0] == 'beq' || reor[0] == 'bne') {
            reodernamento.unshift(reor);
        }
    })
}
...
<th>Entrada</th>
    ...
    resultado += '<table>';
    resultado += ` <tr>
<th>Saida - APENAS BOLHA</th>
</tr> `
    novoValorPiline.forEach((linha) => {
        resultado += `<tr> <td>${linha} </td></tr>`
    })
    resultado += '</table>';
    ...
    resultado += '<table>';
    resultado += ` <tr>
<th>Saida - BOLHA ADIATAMENTO</th>
</tr> `
    bolhaAdiatamento.forEach((linha) => {
        resultado += `<tr> <td>${linha} </td></tr>`
    })
    resultado += '</table>';
    ...
    resultado += '<table>';
    resultado += ` <tr>
<th>Saida - APENAS REORDENAMENTO DE INSTRUÇÕES</th>
</tr> `
    reodernamento.forEach((linha) => {
        resultado += `<tr> <td>${linha} </td></tr>`
    })
    resultado += '</table>';
